...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fetchError) {
      ...
      return NextResponse.json({ error: 'Error al verificar el número de documento' }, { status: 500 });
    }

    if (existingRecords.length > 0) {
      return NextResponse.json({ error: 'Número de documento ya existe' }, { status: 400 });
    }
    ...
    return NextResponse.json({ message: 'Formulario enviado con éxito', data });
  } catch (error) {
    console.error('Error:', error);
    return NextResponse.json({ error: 'Error al subir las imágenes o guardar los datos' }, { status: 500 });
  }
}
